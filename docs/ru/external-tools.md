...
Эта команда только обнаруживает подходящие executables в `PATH` и не запускает их. Используйте `--external-run`, только если явно нужны bounded version/module checks и текущий `PATH` доверенный.
